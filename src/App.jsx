  ...
  return (
    <div className='AppContent full-heght'>
      ...
      <Swipe onSwipeMove={position => windowContext.onSwipeMove(position)} onSwipeEnd={() => windowContext.onSwipeEnd()}>
        <Sidebar.Pushable className='AppContent-pushable full-heght' as={Segment}>
          <SideBar {...windowContext} store={store} />
          <Sidebar.Pusher
            dimmed={windowContext.isMobile && windowContext.isSidebarVisible}
            className={`AppContent-pushable__pusher full-heght ${windowContext.isMobile ? '' : 'appDesctop'}`}
          >
            <TopBar />
