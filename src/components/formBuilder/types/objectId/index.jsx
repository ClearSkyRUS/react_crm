...
export default ({ multi, modelName, value, setValue, field }) => {
    ...
    if (multi && (value === null || typeof value !== 'object' || !Array.isArray(value))) {
        setValue([])
        return ''
    }
    ...
    const getPage = (page, withSearch) => {
        ...
        request(
            modelsApi,
            'get',
            `?model=${model.name}&search=${withSearch !== undefined ? withSearch : search}&page=${page}&limit=5`,
            null,
    ...
    return (
        <Form.Group inline>
            {model !== null ? (
                <Modal model={model.name} field={field} clear={clear} search={search} setSearch={onSearch} multi={multi} selected={multi ? value.length : (value !== null).toString()}>
                    {pages && pages.length && pages[0].docs.length ? <Page model={model} pages={pages} onBottom={onBottom} selected={value} onSelect={onSelectItem} multi={multi} /> : ''}
                    {onLoad ?
                        <Segment style={{ height: '100px' }} >
                            <Dimmer active inverted>
                                <Loader inverted active inline='centered'>Loading</Loader>
                            </Dimmer>
                        </Segment>
                        : ''}
                </Modal>
