...
export default ({ newObject, setNewObject, type, field }) => {
    ...
    return (
        <ModalRedactor type={type} field={field} setNewObject={setNewObject} setKeys={setKeys} add={add} clear={clear}>
            <Card.Group centered>
                {keys.map((field, key) =>
                    [<CardObject key={key} newObject={newObject} setNewObject={setNewObject} field={field} setKeys={setKeys} deleteKey={deleteKey} />]
                )}
            </Card.Group>
        </ModalRedactor>
    )
