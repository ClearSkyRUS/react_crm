...
export default ({newObject, setNewObject, field, setKeys, deleteKey}) => {
	...
	const [type, setType] = useState(
		newObject[field] || newObject[field] === false
			? getCapitalizedType(newObject[field])
			: null
	)
	const [arrayType, setArrayType] = useState(
		newObject[field] || newObject[field] === false
			? getCapitalizedArrayType(newObject[field])
			: null
	)
	const options = Object.keys(typesList).map((key) => ({
		key: typesList.key,
		text: key,
		value: key
	}))
	...
	const onTypeChange = ({value}) => {
		let objectHolder = newObject
		objectHolder[keyName] =
			typeof typesList[value].default === "object"
				? JSON.parse(JSON.stringify(typesList[value].default))
				: typesList[value].default
		setNewObject(objectHolder)
		setType(value)
	}
	return (
			...
			<Card.Content>
				<Form.Group widths="equal">
					<Form.Input
						...
						value={keyName}
						placeholder="Key"
						onChange={(e, data) => onKeyInput(data)}
					/>
					<Form.Select
						fluid
						search
						...
						{type === "Array" ? (
							<Form.Select
								...
								options={options.filter(
									(obj) => obj.value !== "Array" && obj.value !== "Boolean"
								)}
								placeholder="Type"
...
const getCapitalizedArrayType = (value) => {
	if (value.length) {
		...
		return (
			arrayItemType && arrayItemType[0].toUpperCase() + arrayItemType.slice(1)
		)
	} else return null
}
